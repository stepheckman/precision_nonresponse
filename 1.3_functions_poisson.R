...
pop.fn <- function(covar, delta, rpvar, yvar) {
  ...
  # X1 will be transformed into Poisson(Yvar)
...
pop.clust.fn <- function(covar, delta, rpvar, yvar, clustsd) {
  ...
  for (i in 1:A) {
    ...
    # bounding cluster mean between 0 and 1
    cl.effect <- rnorm(1,mean=0,sd=clustsd) 
    clustmean <- yvar + cl.effect
    ...
    yz <- x1x2 %>%
      ...
      #apply inverse CDF to create new marginal distributions for Y and Z
      #introduce between cluster variation here
      mutate(y = qpois(x1.unif, lambda = clustmean),
             z = qnorm(x2.unif, mean=delta, sd=sqrt(rpvar)))
...
loop <- function(rho, delta, lambda, yvar, i) {
    ...
    if (lambda == 0) {
      ...
      popdt <- pop.fn(rho, delta, rpvar, yvar)
    } else {
      # make clustered population according to these settings
      popdt <- pop.clust.fn(rho, delta, rpvar, yvar, lambda)
  ...
  saveRDS(popdt,file=paste("./poisson/pop", i, ".rds", sep = ""))
  ...
  saveRDS(samp.all2,
          file=paste("./poisson/samps_pop", i, ".rds", sep = ""))
  ...
  saveRDS(samplevel.results2,
          file=paste("./poisson/samps_sum", i, ".rds", sep = ""))
  
  cat(sprintf("Done sampling pop %s\n", i))
